...
package org.loocsij._3party.upload;
...
public class File {
	...
	public void saveAs(String destFilePathName) throws SmartUploadException,
			IOException {
	...
	public void fileToField(ResultSet rs, String columnName)
		...
		numBlocks = BigInteger.valueOf(m_size).divide(
				BigInteger.valueOf(blockSize)).longValue();
		leftOver = BigInteger.valueOf(m_size)
				.mod(BigInteger.valueOf(blockSize)).intValue();
		try {
			for (int i = 1; i < numBlocks; i++) {
				rs.updateBinaryStream(columnName, new ByteArrayInputStream(
						m_parent.m_binArray, pos, blockSize), blockSize);
				pos = pos != 0 ? pos : 1;
				pos = i * blockSize;
			}
			...
			if (leftOver > 0)
				rs.updateBinaryStream(columnName, new ByteArrayInputStream(
						m_parent.m_binArray, pos, leftOver), leftOver);
